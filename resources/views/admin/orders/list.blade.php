                ...
                <table id="datatable" class="table table-bordered table-hover table-striped table-sm dataTable no-footer">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @foreach($datatable as $field)
                                @php
                                    $customer = json_decode($record->customer, true)
                                @endphp
                                @if($field == 'amount')
                                ...
                                @elseif($field == 'status')
                                    <td>
                                        @if($record->status == 'pending')
                                            <span class="badge badge-pill badge-warning">{{ __('admin.' . $module . '.statuses.' . $record->status) }}</span>
                                        @elseif($record->status == 'processing')
                                            <span class="badge badge-pill badge-primary">{{ __('admin.' . $module . '.statuses.' . $record->status) }}</span>
                                        @elseif($record->status == 'completed')
                                            <span class="badge badge-pill badge-success">{{ __('admin.' . $module . '.statuses.' . $record->status) }}</span>
                                        @elseif($record->status == 'failed')
                                            <span class="badge badge-pill badge-secondary">{{ __('admin.' . $module . '.statuses.' . $record->status) }}</span>
                                        @else
                                            <span class="badge badge-pill badge-danger">{{ __('admin.' . $module . '.statuses.' . $record->status) }}</span>
                                        @endif
                                    </td>
                                @else
                                    <td>{{ $record->$field }}</td>
                            ...
                            <td class="text-center">
                                {{-- <a class="btn btn-info btn-sm" href="{{ route('admin.' . $module . '.edit', ['order' => $record]) }}">
                                    <i class="fas fa-pencil-alt"></i>
                                </a> --}}
                                <a class="btn btn-info btn-sm" href="{{ route('admin.' . $module . '.show', ['order' => $record]) }}">
                                    <i class="fas fa-eye"></i>
                                </a>
                                {{-- <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#confirmModal" data-url="{{ route('admin.' . $module . '.destroy', ['order' => $record]) }}">
