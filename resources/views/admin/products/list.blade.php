                ...
                <table id="datatable" class="table table-bordered table-hover table-striped table-sm dataTable no-footer">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @foreach($datatable as $field)
                                @if($field == 'is_active')
                                <td><span class="badge badge-pill badge-{{ $record->$field ? 'primary' : 'secondary' }}">{{ $record->$field ? __('Active') : __('Inactive') }}</span></td>
                                @elseif($field == 'price')
                                <td>{{ number_format($record->$field, 2) }}</td>
                                @elseif($field == 'image')
                                <td class="text-center"><img src="{{ $record->images()->first()?->filepath ? Storage::url($record->images()->first()?->filepath) : asset('img/no-image.jpg') }}" width="40" alt="{{ $record->name }} image"></td>
                            ...
                            <td class="text-center">
                                <a class="btn btn-info btn-sm" href="{{ route('admin.' . $module . '.edit', ['product' => $record]) }}">
                                    <i class="fas fa-pencil-alt"></i>
                                </a>
                                <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#confirmModal" data-url="{{ route('admin.' . $module . '.destroy', ['product' => $record]) }}">
